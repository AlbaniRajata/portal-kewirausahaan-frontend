...
export default function RegisterMahasiswaPage() {
  ...
  const handleFileChange = (e) => {
    ...
    if (!allowedFormats.includes(file.type)) {
      setErrors((prev) => ({
        ...prev,
        foto_ktm: "Format file harus JPG, JPEG, PNG, atau PDF",
      }));
      return;
    }
    ...
    if (file.size > maxSize) {
      setErrors((prev) => ({
        ...prev,
        foto_ktm: "Ukuran file maksimal 10MB",
      }));
      return;
    }
  ...
  const validate = () => {
    ...
    if (!form.tahun_masuk) {
      newErrors.tahun_masuk = "Tahun masuk wajib diisi";
    } else if (!/^\d{4}$/.test(form.tahun_masuk)) {
      newErrors.tahun_masuk = "Format tahun tidak valid (contoh: 2023)";
    }
    ...
    if (!form.foto_ktm) {
      newErrors.foto_ktm = "Foto KTM wajib diupload";
    }
    ...
    <Box sx={{ minHeight: "100vh", display: "flex" }}>
      ...
      >
        <Paper elevation={0} sx={{ width: "100%", maxWidth: 540, p: 4 }}>
          ...
          {errors.foto_ktm && (
            <Typography sx={{ color: "#d32f2f", fontSize: 12, mb: 1, ml: 1.5 }}>
              {errors.foto_ktm}
            </Typography>
          )}
          ...
          {form.foto_ktm && form.foto_ktm.type === "application/pdf" && (
            <Typography sx={{ fontSize: 12, color: "#666", mb: 2, ml: 1.5 }}>
              File PDF terpilih: {form.foto_ktm.name}
            </Typography>
          )}
