...
import {
  ...
  Autocomplete, CircularProgress, InputAdornment, IconButton,
} from "@mui/material";
...
import { getProfile, updateProfile, updatePassword, deleteProfilePhoto } from "../../api/dosen";
...
export default function BiodataDosenPage() {
  ...
  useEffect(() => { fetchProfile(); fetchJurusan(); fetchProdi(); }, []);
  ...
  const fetchJurusan = async () => {
    try {
      ...
      if (response.success) setJurusanOptions(response.data.map((item) => ({ label: item.nama_jurusan, id: item.id_jurusan })));
    } catch { setJurusanOptions([]); } finally { setLoadingJurusan(false); }
  };
  ...
  const fetchProdi = async () => {
    try {
      ...
      if (response.success) setProdiOptions(response.data.map((item) => ({
        label: `${item.jenjang} ${item.nama_prodi} - ${item.nama_jurusan} (${item.nama_kampus})`,
        id: item.id_prodi,
      })));
    } catch { setProdiOptions([]); } finally { setLoadingProdi(false); }
  };

  const handleChangeBiodata  = (field, value) => { setFormBiodata((p) => ({ ...p, [field]: value })); setErrors((p) => ({ ...p, [field]: "" })); };
  const handleChangePassword = (field, value) => { setFormPassword((p) => ({ ...p, [field]: value })); setErrors((p) => ({ ...p, [field]: "" })); };
  ...
  const handleFileChange = (e) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      setErrors((p) => ({ ...p, foto: "Ukuran file maksimal 5MB" })); return;
    }
  ...
  const validateBiodata = () => {
    const e = {};
    if (!formBiodata.nama_lengkap) e.nama_lengkap = "Nama lengkap wajib diisi";
    if (!formBiodata.no_hp)        e.no_hp        = "Nomor WhatsApp wajib diisi";
    else if (!/^08[0-9]{8,11}$/.test(formBiodata.no_hp)) e.no_hp = "Format nomor tidak valid (08xxxxxxxxxx)";
    setErrors(e);
  ...
  if (loading) return (
    <BodyLayout Sidebar={DosenNavbar}>
      <Box sx={{ position: "relative", minHeight: "60vh" }}>
        <LoadingScreen message="Memuat biodata dosen..." overlay minHeight="60vh" />
      </Box>
    </BodyLayout>
  );
          ...
          <Paper elevation={0} sx={{ mb: 3, borderRadius: "20px", border: "1.5px solid #E5E7EB", overflow: "hidden" }}>
            ...
            <Box sx={{ p: { xs: 2.5, sm: 4 } }}>
              <SectionHeader
                ...
                subtitle="Data diri dan kontak dosen"
              ...
              <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr" }, gap: 3, mb: 3 }}>
                ...
                <Box>
                  ...
                  <ReadonlyField value={formBiodata.nip} />
                </Box>
              </Box>
              ...
              <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr" }, gap: 3, mb: 3 }}>
                ...
                <Box>
                  ...
                  {loadingJurusan
                    ? <Box sx={{ display: "flex", alignItems: "center", py: 2 }}><CircularProgress size={22} sx={{ color: COLORS.primary }} /></Box>
                    : <Autocomplete
                        options={jurusanOptions} value={formBiodata.id_jurusan}
              ...
              <Box sx={{ display: "grid", gridTemplateColumns: "1fr", gap: 3, mb: 3 }}>
                <Box>
                  ...
                  {loadingProdi
                    ? <Box sx={{ display: "flex", alignItems: "center", py: 2 }}><CircularProgress size={22} sx={{ color: COLORS.primary }} /></Box>
                    : <Autocomplete
                        options={prodiOptions} value={formBiodata.id_prodi}
              ...
              <Box sx={{ display: "flex", justifyContent: "flex-end" }}>
                <Button
                  ...
                  sx={{
                    px: 4, py: 1.3, textTransform: "none", fontWeight: 700, borderRadius: "12px", fontSize: 14,
                    background: `linear-gradient(135deg, ${COLORS.primary} 0%, ${COLORS.accent} 100%)`,
                    boxShadow: `0 4px 15px rgba(13,89,242,0.35)`,
                    "&:hover": { background: `linear-gradient(135deg, ${COLORS.primaryDark} 0%, ${COLORS.primary} 100%)`, boxShadow: `0 6px 20px rgba(13,89,242,0.45)` },
                    ...
                    InputProps={{
                      endAdornment: (
                        <InputAdornment position="end">
                          <IconButton onClick={() => setShowPassword((p) => ({ ...p, [key]: !p[key] }))} edge="end" size="small">
                            {showPassword[key]
              ...
              <Box sx={{ display: "flex", justifyContent: "flex-end", mt: 1 }}>
                <Button
                  ...
                  sx={{
                    ...
                    "&:hover": { background: `linear-gradient(135deg, ${COLORS.primaryDark} 0%, ${COLORS.secondary} 100%)`, boxShadow: `0 6px 20px rgba(37,99,235,0.45)` },
                    "&:disabled": { background: "#E5E7EB", color: "#9CA3AF", boxShadow: "none" },
                  }}
                >
