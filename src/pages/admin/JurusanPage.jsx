...
export default function JurusanPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      Swal.fire({ icon: "error", title: "Gagal", text: "Gagal memuat data jurusan", confirmButtonColor: "#0D59F2" });
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (currentDialog.mode === "create") {
        await createJurusan(currentForm);
      } else {
        await updateJurusan(currentDialog.data.id_jurusan, currentForm);
      }
      await Swal.fire({ icon: "success", title: "Berhasil", text: currentDialog.mode === "create" ? "Jurusan berhasil ditambahkan" : "Jurusan berhasil diperbarui", timer: 2000, timerProgressBar: true, showConfirmButton: false });
      fetchData();
    } catch (err) {
      Swal.fire({ icon: "error", title: "Gagal", text: err.response?.data?.message || "Gagal menyimpan jurusan", confirmButtonColor: "#0D59F2" });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDelete = async (item) => {
    ...
    try {
      await deleteJurusan(item.id_jurusan);
      await Swal.fire({ icon: "success", title: "Berhasil", text: "Jurusan berhasil dihapus", timer: 2000, timerProgressBar: true, showConfirmButton: false });
      fetchData();
    } catch (err) {
      Swal.fire({ icon: "error", title: "Gagal", text: err.response?.data?.message || "Gagal menghapus jurusan", confirmButtonColor: "#0D59F2" });
    }
  };
      ...
      <PageTransition>
        <Box sx={{ mb: 4 }}>
          ...
          <Typography sx={{ fontSize: 16, color: "#6B7280" }}>
            Kelola data jurusan yang terdaftar dalam sistem portal kewirausahaan
          </Typography>
        </Box>
          ...
          <Box sx={{ p: { xs: 3, md: 4 } }}>
            ...
            {loading ? (
              <Box sx={{ position: "relative", minHeight: 400 }}>
                <LoadingScreen message="Memuat data jurusan..." overlay minHeight="400px" />
              </Box>
                ...
                <Paper elevation={0} sx={{ p: { xs: 5, sm: 10 }, textAlign: "center", borderRadius: "20px", border: "1.5px solid #E2E8F0", backgroundColor: "#F8FAFC" }}>
                  ...
                  <Typography sx={{ fontSize: { xs: 14, sm: 16 }, color: COLORS.slate, fontWeight: 500 }}>
                    {search ? "Coba gunakan kata kunci pencarian lainnya" : "Klik tombol Tambah Jurusan untuk mulai menambahkan data"}
                  </Typography>
                </Paper>
            ) : (
              <>
                     ...
                     <TableBody>
                       {paginatedList.map((item, idx) => (
                         <TableRow key={item.id_jurusan} sx={tableBodyRow}>
                           ...
                           <TableCell>
                             <Typography sx={{ fontWeight: 700, fontSize: { xs: 13, sm: 15 }, color: "#1E293B" }}>{item.nama_jurusan}</Typography>
                           </TableCell>
                           <TableCell>
        ...
        <Dialog open={dialog.open} onClose={handleCloseDialog} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: { xs: "16px", sm: "24px" }, overflow: "hidden" } }}>
          <Box sx={{ p: { xs: 1.5, sm: 2 }, display: "flex", alignItems: "center", justifyContent: "space-between", background: `linear-gradient(135deg, ${COLORS.primary}, ${COLORS.accent})`, color: "#fff" }}>
            <Box sx={{ display: "flex", alignItems: "center", gap: 1.5, px: 1 }}>
              <Typography sx={{ fontWeight: 800, fontSize: { xs: 16, sm: 18 } }}>
                {dialog.mode === "create" ? "Tambah Data Jurusan" : "Edit Data Jurusan"}
              </Typography>
            </Box>
          ...
          <DialogContent sx={{ px: { xs: 2.5, sm: 4 }, py: { xs: 3, sm: 4 } }}>
            <Box>
              <Typography sx={{ fontSize: 13, fontWeight: 700, mb: 1, color: "#334155" }}>Nama Jurusan <span style={{ color: COLORS.error }}>*</span></Typography>
              <TextField
