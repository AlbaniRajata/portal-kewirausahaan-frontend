...
import {
  ...
  Autocomplete, CircularProgress, Alert, InputAdornment, IconButton,
} from "@mui/material";
...
import { getProfile, updateProfile, updatePassword, deleteProfilePhoto } from "../../api/mahasiswa";
...
export default function BiodataMahasiswaPage() {
  ...
  const fetchJurusan = async () => {
    ...
    } catch {} finally { setLoadingJurusan(false); }
  };
  ...
  const fetchProdi = async () => {
    try {
      ...
      if (response.success) setProdiOptions(response.data.map((item) => ({
        label: `${item.jenjang} ${item.nama_prodi} - ${item.nama_jurusan} (${item.nama_kampus})`, id: item.id_prodi,
      })));
    } catch {} finally { setLoadingProdi(false); }
  };
  ...
  const validateBiodata = () => {
    ...
    if (!formBiodata.id_jurusan) e.id_jurusan = "Jurusan wajib dipilih";
    if (!formBiodata.id_prodi)   e.id_prodi   = "Program studi wajib dipilih";
    setErrors(e);
  ...
  if (loading) return (
    <BodyLayout Sidebar={MahasiswaNavbar}>
      <Box sx={{ position: "relative", minHeight: "60vh" }}>
        <LoadingScreen message="Memuat biodata mahasiswa..." overlay minHeight="60vh" />
      </Box>
    </BodyLayout>
  );
          ...
          <Paper elevation={0} sx={{
            ...
            <Box sx={{ p: { xs: 2.5, sm: 4 } }}>
              <SectionHeader
                ...
                subtitle="Data diri dan kontak mahasiswa"
              ...
              <Box sx={{ mb: 4 }}>
                ...
                <TextField
                  ...
                  disabled={submitting}
                  sx={{ "& .MuiOutlinedInput-root": { borderRadius: "12px", backgroundColor: "#fff", "&:hover fieldset": { borderColor: COLORS.primary }, "&.Mui-focused fieldset": { borderColor: COLORS.primary }, "&.Mui-focused": { boxShadow: `0 0 0 3px ${COLORS.primaryLight}` } } }}
                />
              </Box>
