...
const getStatusInfo = (status, peran) => {
  if (peran === 1)
    return { label: "Otomatis", color: "#e8f5e9", bg: "#2e7d32" };
  switch (status) {
    case 0:
      return { label: "Menunggu", color: "#fff8e1", bg: "#f57f17" };
    case 1:
      return { label: "Disetujui", color: "#e8f5e9", bg: "#2e7d32" };
    case 2:
      return { label: "Ditolak", color: "#fce4ec", bg: "#c62828" };
    default:
      return { label: "Unknown", color: "#f5f5f5", bg: "#666" };
  }
};
...
export default function AnggotaTimPage() {
  ...
  const handleRemoveAnggota = (index) => {
    setFormTim({
      ...formTim,
      anggota: formTim.anggota.filter((_, i) => i !== index),
    });
  };
  ...
  if (
    timStatus?.hasTim &&
    (timStatus?.isKetua ||
      (timStatus?.isAnggota && timStatus?.statusAnggota === 1))
  ) {
    const allApproved = timDetail?.anggota?.every(
      (item) => item.peran === 1 || item.status === 1,
    );
    ...
    return (
      <BodyLayout Sidebar={SidebarMahasiswa}>
        <Box>
          ...
          <Paper
            sx={{ p: 4, borderRadius: "16px", border: "1px solid #f0f0f0" }}
          >
            <Box
              sx={{
                display: "grid",
            ...
            >
              <Table>
                <TableHead>
                  <TableRow>
                    {["Nama Lengkap", "NIM", "Prodi", "Peran", "Status"].map(
                      (h, i) => (
                  ...
                  {timDetail?.anggota?.map((item, index) => {
                    ...
                    return (
                      <TableRow key={index} sx={tableBodyRow}>
                        ...
                        <TableCell>
                          <StatusPill
                            label={peranInfo.label}
                            bg={peranInfo.bg}
                            color={peranInfo.color}
                          />
                        </TableCell>
                        <TableCell>
                          <StatusPill
        ...
        <Paper sx={{ p: 4, borderRadius: "16px", border: "1px solid #f0f0f0" }}>
          ...
          {formTim.anggota.map((item, index) => (
            ...
            >
              ...
              >
                <Typography
                  sx={{ fontWeight: 700, fontSize: 14, color: "#444" }}
                >
                  Anggota {index + 1}
                </Typography>
                ...
                <Box>
                  ...
                  <Autocomplete
                    ...
                    onInputChange={(e, value) =>
                      handleSearchMahasiswa(index, value)
                    }
                    onChange={(e, value) => {
                          ...
                          endAdornment: (
                            <>
                              {searching ? (
                                ...
                                <Search sx={{ color: "#bbb", fontSize: 20 }} />
                              )}
            ...
            <Button
              ...
              sx={{
                ...
                "&:hover": {
                  ...
                  borderColor: "rgba(13,89,242,0.6)",
                },
              }}
            >
              Tambah Anggota
