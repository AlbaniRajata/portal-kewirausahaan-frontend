...
export default function DashboardReviewerPage() {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      const [profileRes, tahap1Res, tahap2Res] = await Promise.allSettled([
        getProfile(),
        getListPenugasan(1),
        getListPenugasan(2),
      ]);
      ...
      const tahap1 = tahap1Res.status === "fulfilled"
        ? extractList(tahap1Res).map((p) => ({ ...p, tahap: 1 }))
        : [];
      const tahap2 = tahap2Res.status === "fulfilled"
        ? extractList(tahap2Res).map((p) => ({ ...p, tahap: 2 }))
        : [];

      setData({
        profile: profileRes.status === "fulfilled" ? profileRes.value.data : null,
        penugasan: [...tahap1, ...tahap2],
      });
      setLoading(false);
    };
    fetchAll();
  ...
  const statCards = [
    {
      ...
      sub: `${penugasan.filter((p) => p.tahap === 1).length} desk · ${penugasan.filter((p) => p.tahap === 2).length} wawancara`,
      accent: "#0D59F2",
            ...
            <Box sx={{ display: "flex", alignItems: "center", gap: 3, position: "relative", zIndex: 1 }}>
              ...
              <Box>
                ...
                <Typography sx={{ fontSize: 24, fontWeight: 800, lineHeight: 1.2 }}>
                  {profile?.nama_lengkap || user?.nama_lengkap || "Reviewer"}
                </Typography>
