...
import { getListProposalRekapTahap1, finalisasiDeskBatch } from "../../api/admin";
...
const getStatusInfo = (status, totalReviewer, totalSubmit) => {
  ...
  if (status === 2) {
    if (totalSubmit === totalReviewer && totalReviewer > 0) return { text: "Menunggu Finalisasi", color: "warning" };
...
export default function RekapTahap1Tab({ id_program }) {
  ...
  const handleFinalisasi = async (isLolos) => {
    ...
    try {
      ...
      await finalisasiDeskBatch(id_program, payload);
    ...
    <Box>
      ...
      {loading ? (
        ...
        <Box sx={{ textAlign: "center", py: 8 }}>
          <Typography sx={{ fontSize: 14, color: "#999" }}>Belum ada proposal untuk tahap desk evaluasi</Typography>
        </Box>
      ) : (
        <>
            ...
            <Table>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell sx={tableHeadCell}>Penilaian Submit</TableCell>
                ...
                {paginated.map((p) => {
                  ...
                  return (
                    <TableRow key={p.id_proposal} sx={tableBodyRow} hover selected={isSelected}>
                      ...
                      <TableCell>
                        <Chip label={`${p.total_submit} / ${p.total_reviewer}`} size="small"
      ...
      <DetailRekapDialog open={dialogOpen} onClose={() => setDialogOpen(false)}
        ...
        judul={selectedProposal?.judul} tahap={1} />
    </Box>
  );
}