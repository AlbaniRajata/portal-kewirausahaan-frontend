...
import { getPreviewDistribusi, executeAutoDistribusi } from "../../api/admin";
...
export default function DistribusiOtomatisTab({
  ...
  useEffect(() => {
    const fetchPreview = async () => {
      ...
      try {
        setLoading(true);
        setErrorMsg("");
        const response = await getPreviewDistribusi(id_program, tahap);
  ...
  const handleExecute = async () => {
    const result = await Swal.fire({
      ...
      html: `Anda akan mendistribusikan <b>${preview.total_proposal}</b> proposal ke <b>${preview.total_reviewer}</b> reviewer secara otomatis.<br/><br/>Lanjutkan?`,
      icon: "question",
    ...
    if (result.isConfirmed) {
      try {
        setExecuting(true);
        const response = await executeAutoDistribusi(id_program, tahap);
    ...
    return (
      <Box sx={{ textAlign: "center", py: 5 }}>
          ...
          <Typography sx={{ fontSize: 14, color: "#999", mb: 3 }}>
            Belum ada proposal dengan status "Diajukan" yang dapat didistribusikan
          </Typography>
          <Button
            variant="outlined"
            startIcon={<Refresh />}
