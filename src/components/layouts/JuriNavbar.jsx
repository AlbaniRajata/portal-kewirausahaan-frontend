...
export default function JuriNavbar() {
  ...
  const activeMenu = useMemo(() => {
    ...
    if (path.startsWith("/juri/dashboard")) return "Dashboard";
    if (path.startsWith("/juri/biodata")) return "Biodata";
    if (path.startsWith("/juri/penugasan")) return "Penugasan Saya";
    return "";
  }, [location.pathname]);
  ...
  const menuItems = [
    { text: "Dashboard", path: "/juri/dashboard", icon: <DashboardIcon sx={{ fontSize: 20 }} /> },
    { text: "Biodata", path: "/juri/biodata", icon: <PersonIcon sx={{ fontSize: 20 }} /> },
    { text: "Penugasan Saya", path: "/juri/penugasan", icon: <AssignmentIcon sx={{ fontSize: 20 }} /> },
  ];
...
JuriNavbar.hideInBodyLayout = true;
JuriNavbar.renderAsNavbar = true;