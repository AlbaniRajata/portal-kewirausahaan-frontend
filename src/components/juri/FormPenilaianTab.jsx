...
import { getFormPenilaian, simpanNilai, submitPenilaian, resetPenilaian } from "../../api/juri";
...
export default function FormPenilaianTab({ id_distribusi, onActionsChange }) {
  ...
  const fetchFormPenilaian = useCallback(async () => {
    try {
      ...
      if (response.success) {
        ...
        const localDraft = localStorage.getItem(`penilaian_draft_juri_${id_distribusi}`);
        if (localDraft) {
          try {
  ...
  useEffect(() => {
    if (Object.keys(formData).length > 0) {
      localStorage.setItem(`penilaian_draft_juri_${id_distribusi}`, JSON.stringify(formData));
      setLastAutoSave(new Date());
    }
  ...
  const handleSimpanDraft = async () => {
    ...
    try {
      ...
      if (response.success) {
        localStorage.removeItem(`penilaian_draft_juri_${id_distribusi}`);
